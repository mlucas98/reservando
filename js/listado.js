...
Listado.prototype.buscarRestaurante = function(id) {
    let restaurant = this.restaurantes.find(elemento => elemento.id == id);   //find elemento (que pertenece al arreglo), donde elemento.id sea igual al id del parametro
...
Listado.prototype.filtrar = function(arreglo){
    let arreglo2 = arreglo.filter(function(elem, index, self){  //devuelve un nuevo arreglo con lo filtrado del arreglo pasado en la función
        return index === self.indexOf(elem);
    })
    return arreglo2.sort();
}

Listado.prototype.mapear = function(arregloDeObjetos, queBusca){    //encontrar el queBusca en cada
    let arreglo;
    switch (queBusca) {
    case 'ubicacion':
        arreglo = arregloDeObjetos.map(function(elemento){ //elemento hace referencia a cada elemento de arregloDeObjetos
            return elemento.ubicacion;
        });
        break;
    case 'rubro':
        arreglo = arregloDeObjetos.map(function(elemento){
    ...
    /*
    let arreglo = arregloDeObjetos.map(function(elemento){
        return elemento.queBusca;
    })
    */
    return arreglo;
}
...
Listado.prototype.obtenerCiudad = function() {
    ...
    let ciudades = this.mapear(this.restaurantes, 'ubicacion');
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    let ciudadesFiltradas = this.filtrar(ciudades);
...
Listado.prototype.obtenerRubro = function() {
    let rubros = this.mapear(this.restaurantes, 'rubro');
    let rubroFiltrado = this.filtrar(rubros);
    return rubroFiltrado;
}
...
Listado.prototype.obtenerHorario = function() {
    ...
    let arregloHorarios = this.mapear(this.restaurantes, 'horarios');

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horariosIndividual = [];
    arregloHorarios.forEach(function(horarioEnArreglo) {
        horarioEnArreglo.forEach(function(horario) {
            horariosIndividual.push(horario)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    let horariosFiltrados = this.filtrar(horariosIndividual);
